...
for i = 1:nParticles
    ...
    pBestVector(i,dimension+1) = rosenbrock(swarm(i).position);
end
...
for i = 1:nIterations
    ...
    for j = 1:nParticles
        ...
        if(rosenbrock(swarm(j).position) < pBestVector(j,dimension+1))
            ...
            pBestVector(j,dimension+1) = rosenbrock(swarm(j).position);
